...
import { estimateDataUrlBytes, haversineKm, severityIndex, withinJamaica } from './utils.js';
...
function createSeedIncidents(users) {
  ...
  const samples = [
    ...
    },
    {
      ...
      description:
        'Multi-vehicle collision on the North-South Highway near Vineyards toll. Emergency services on scene.',
      severity: 'critical',
      parish: 'St. Catherine',
      latitude: 18.0456,
      longitude: -77.0123
    },
    {
      ...
      description:
        'Break-in reported at a business on Constant Spring Road. Police investigating.',
      severity: 'medium',
      parish: 'St. Andrew',
      latitude: 18.0312,
      longitude: -76.7845
    ...
    {
      category: 'Infrastructure',
      subcategory: 'Water Main Break',
      description:
        'Major water main break on Old Hope Road causing flooding and traffic delays.',
      severity: 'high',
      parish: 'Kingston',
      latitude: 18.0089,
      longitude: -76.7512
    },
    {
      ...
      description:
        'Landslide blocking sections of the Junction Road in Portland. Motorists should use alternate routes.',
      severity: 'critical',
      parish: 'Portland',
      latitude: 18.1876,
      longitude: -76.4523
    },
    {
      ...
      description:
        'Missing elderly woman last seen near Papine market. Family asking for community assistance.',
      severity: 'high',
      parish: 'St. Andrew',
      latitude: 18.0234,
      longitude: -76.7389
    ...
    {
      category: 'Accident',
      subcategory: 'Pedestrian Incident',
      description:
        'Pedestrian struck by vehicle near Cross Roads. Traffic being diverted.',
      severity: 'high',
      parish: 'Kingston',
      latitude: 17.9923,
      longitude: -76.7856
    },
    {
      ...
      subcategory: 'Assault',
      description:
        'Assault reported outside a nightclub on Knutsford Boulevard. Police presence increased in the area.',
      severity: 'medium',
      parish: 'Kingston',
      latitude: 18.0012,
      longitude: -76.7934
    },
    {
      ...
      description:
        'Large pothole causing accidents on Washington Boulevard. NWA notified.',
      severity: 'medium',
      parish: 'St. Andrew',
      latitude: 18.0178,
      longitude: -76.8234
    },
    {
      ...
      description:
        'Minor earthquake felt across Kingston and St. Andrew. No major damage reported.',
      severity: 'low',
      parish: 'Kingston',
      latitude: 17.9712,
      longitude: -76.7928
    },
    {
      ...
      description:
        'Suspicious individuals reported canvassing homes in Cherry Gardens. Residents advised to be vigilant.',
      severity: 'medium',
      parish: 'St. Andrew',
      latitude: 18.0267,
      longitude: -76.7712
    },
    {
      ...
      subcategory: 'Fire',
      description:
        'Structure fire at a warehouse in Newport West. Fire brigade responding. Smoke visible from downtown.',
      severity: 'critical',
      parish: 'Kingston',
      latitude: 17.9634,
      longitude: -76.8123
  ...
  return samples.map((sample, index) => ({
    ...
    status: index === 2 || index === 7 ? 'responding' : 'active',
    credibility: index < 4 ? 'verified' : 'pending',
    confirmationCount: index < 4 ? 5 : Math.floor(Math.random() * 4) + 1,
    disputeCount: 0,
    anonymous: index === 3 || index === 8,
    photos: [],
        ...
        ? [
            {
              ...
              action: 'respond',
              notes: index === 2 ? 'Rescue team deployed and traffic rerouted.' : 'Road clearing crew dispatched.',
              createdAt: new Date(now - 3 * 60 * 60 * 1000).toISOString()
            }
          ]
        : [],
    createdAt: new Date(now - (index + 1) * 2 * 60 * 60 * 1000).toISOString(),
    updatedAt: new Date(now - index * 60 * 60 * 1000).toISOString()
  }));
}
...
export async function createIncident(payload, requesterId) {
  return withDb(async (db) => {
    ...
    for (const photo of Array.isArray(photos) ? photos.slice(0, INCIDENT_PHOTO_UPLOAD.maxCount) : []) {
      const isSupported = INCIDENT_PHOTO_UPLOAD.allowedTypes.includes(photo.type);
      const effectiveSize = Math.max(
        Number(photo.size || 0),
        estimateDataUrlBytes(photo.dataUrl)
      );
      const isSmallEnough =
        effectiveSize > 0 && effectiveSize <= INCIDENT_PHOTO_UPLOAD.maxBytes;
      if (!isSupported || !isSmallEnough) {
        throw new PlatformError(
          400,
          `Photos must be JPEG or PNG and ${Math.round(INCIDENT_PHOTO_UPLOAD.maxBytes / 1024)} KB or less.`,
          4016
        );
      }
      try {
