...
function createNotificationRecord(db, user, incident) {
  ...
  db.notifications.push({
    ...
    body: `${incident.address || `${incident.parish}, Jamaica`} • Reported by ${
...
export async function getAuthorityDashboard(requesterId, parishOverride = '', statusFilter = '') {
  ...
  const parish = user.role === 'admin' ? parishOverride || user.parish : user.parish;
  let incidents = db.incidents.filter((incident) => incident.parish === parish);
  if (statusFilter) {
