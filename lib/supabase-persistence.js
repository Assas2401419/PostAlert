...
const USER_TABLE_CANDIDATES = ['postalert_users', 'jeip_users'];
...
async function resolveUsersTable(client) {
  for (const table of USER_TABLE_CANDIDATES) {
    ...
    if (!isMissingTableError(result.error)) {
      throw new Error(`[supabase] inspect ${table}: ${result.error.message}`);
    }
  }

  throw new Error(
    `[supabase] Could not find a supported users table. Checked ${USER_TABLE_CANDIDATES.join(', ')}.`
  );
}
...
export async function loadSupabaseDb(createSeedDatabase) {
  ...
  const usersTable = await resolveUsersTable(client);
  ...
  const [
    ...
    client.from(usersTable).select('*').order('created_at', { ascending: true }),
  ...
  const userRows = requireResult(`select ${usersTable}`, usersResult);
  if (!userRows.length) {
...
export async function saveSupabaseDb(db) {
  ...
  await upsertTable(client, usersTable, db.users.map(mapUserToRow));
