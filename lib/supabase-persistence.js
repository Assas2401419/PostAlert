...
export async function loadSupabaseDb(createSeedDatabase) {
  ...
  const [
    ...
    client.from('jeip_users').select('*').order('created_at', { ascending: true }),
  ...
  const userRows = requireResult('select jeip_users', usersResult);
  if (!userRows.length) {
...
export async function saveSupabaseDb(db) {
  ...
  await upsertTable(client, 'jeip_users', db.users.map(mapUserToRow));
